...
export const featureExtractorNode = async (state) => {
...
export const scenePlannerNode = async (state) => {
  ...
  // Auto-repair voiceLines: append period to any fragment missing terminal punctuation
  scenes = scenes.map(sc => {
    const vl = (sc.voiceLine || "").trim();
    return { ...sc, voiceLine: /[.!?]$/.test(vl) ? vl : vl + "." };
  });
